    ...
    >
      ...
      {isVisible && (
        <div className={`absolute z-50 ${positionClasses[position]}`}>
          <div className="bg-gray-800 text-white text-xs rounded-lg py-2 px-3 max-w-xs whitespace-normal">
            {content}
...
export const InfoTooltip: React.FC<InfoTooltipProps> = ({ content, className = '' }) => {
  return (
    <Tooltip content={content} className={className}>
      <Info className="h-4 w-4 text-[#666] cursor-help hover:text-[#00AEEF] transition-colors" />
    </Tooltip>
  );
}; 